function openPage(){
   ...
   let b1 = /bolo encomenda/i
    if (a1.test(answer)  || b1.test(answer)){
        window.location.href = "boloslogged.php";
        }

   let a2 = /doces/i
   let b2 = /doce encomenda/i
    if (a2.test(answer)  || b2.test(answer)){
        window.location.href = "doceslogged.php";
        }
  ...
  let b8 = /contato/i
    ...
    let d10 = /inicio/i
        if(a10.test(answer) || b10.test(answer) || c10.test(answer) || d10.test(answer)){
            window.location.href = "iniciologged.php"
                }
    ...
    let c11 = /missão/i
            if(a11.test(answer) || b11.test(answer) || c11.test(answer)){
                window.location.href = "quemsomoslogged.php"
                }

    let a12 = /perfil/i
    let b12 = /configuração/i
    let c12 = /configurações/i
            if(a12.test(answer) || b12.test(answer) || c12.test(answer)){
                window.location.href = "perfilFeito.php"
                }
  }
